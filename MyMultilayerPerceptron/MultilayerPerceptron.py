...
class Layer:
    ...
    Параметры:
        ...
        w: Хранит веса синапсов между этим слоем и предыдущим
        activation: Хранит название активационной функции данного слоя

    Методы:
        get_value: Возвращает параметры класса в виде словаря
    """
    ...
    def get_value(self):
        """
        :return: Возвращает параметры класса в виде словаря
        """
...
class MultilayerPerceptron:
    """
    Класс MultilayerPerceptron предназначен для послойной реализации многослойного персептрона
    ...
    def add_layer(self, number_neurons, input_layer=False, activation='logistic'):
        ...
        :param input_layer: Содержит булевое значение, которое является ответом на вопрос: "Является ли слой входным?"
    ...
    def __gradient(self, x):
        ...
        :param x: Хранит сигнал, который поступает на входной слой
        :return: Возвращает сигнал (результат метода __signal_accumulation) прошедший через градиент активационной функции
        """
        signals = self.__signal_accumulation(x)
        for i in range(1, self.number_of_layers):
    ...
    def __errors(self, y, output_y):
        ...
        :param y: Теоретический выходной сигнал
    ...
    def __verification_of_compliance(self, x, y=[]):
        ...
        Вспомогательный метод для проверки:
            - количество входных сигналов соответствует количеству нейронов во входном слое
